...
| <img src="${icon}" alt="${name} logo" width="80px" height="auto" /> | ${description} | ${tags.map((tag) => `\`${tag}\``).join(', ')} |
| --- | --- | --- |
...
ResourcesJson.forEach((resource, index) => {
    tableOfContents += index === 0 ? `\t- ${createResourcesTableOfContent(resource)}` : createResourcesTableOfContent(resource);
